package com.dixond.logging {
	...
	*	Note: Flash has no easy way to get it's own ip address. So we may not pass that value in the HOSTNAME part syslog message.
	...
	public class SyslogView extends AbstractLogView{
		...
		// whether this view has been disabled due to source inavailability. (e.g can't connect to syslog server)
		// private var _enabled:Boolean;
		
		/**
		 * Constructor
		 *	aLog - reference to the Logger object we're observing
		 *	aInfo - identifying info that gets sent in the syslog message. Including address (ideally domain or ip address) and TAG (usually the app name).
		 *		the object looks like: {client_address:xxx, app:"appName}. 
		 *		I usually leave client_address empty because 
		...
		public function SyslogView (aLog:Logger, aHost:String, aInfo:Object) {
			super(aLog);
			_socket = new Socket();
			_info = aInfo;
			_host = aHost;
		...
		private function socketConnectHandler(aEvent:Event) {
			enable();
			_log.info("// ======= Connected to syslog daemon at " + _host + " ======= //");
		...
		public function logUpdateHandler (aEvent:LogEvent):void {
			...
			var msg = "<" + priority + ">" + timestamp + SPACE + _info.client_address + SPACE + _info.app + " - " + loggerLevelDesc + ": " + logMsg;
			sendData(msg);
		}
		private function sendData(aMsg:String) {
			// avoid stack overloads on failed logging by disabling this log when it's write target is not available.
			if (_enabled) {
	            try {            
